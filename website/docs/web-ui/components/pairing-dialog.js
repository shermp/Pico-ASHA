  ...
  render() {
    return html`
      <dialog aria-labelledby="pair-title" @cancel=${this.close}>
        ...
        ${this.candidates.length ? html`<div class="list">${this.candidates.map((candidate) => html`<button class="candidate" type="button" ?disabled=${this.busy} @click=${() => this.select(candidate)} aria-label=${`Pair ${candidate.name || candidate.address}`}><span class="identity"><strong>${candidate.name || "Unnamed hearing aid"}</strong><small>${candidate.address}</small></span><span class="signal">${icon("bluetooth_searching")}${candidate.rssi} dBm</span></button>`)}</div>` : html`<p class="empty">No hearing-aid advertisements received yet.</p>`}
      </dialog>
    `;
  }
}
